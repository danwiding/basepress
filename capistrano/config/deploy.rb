...
set :repository, "git@github.com:PolymathicMedia/Junto-Website.git"
set :scm, :git
...
namespace :JuntoDeploy do
      task :CreateSharedFolders, :roles => :app do
            ...
            run "lftp -u '#{ftp_username}','#{ftp_password}' -e \"mkdir -p /remoteftp/juntobackups/#{stage}/db-and-shared/ondeploy;mkdir /remoteftp/juntobackups/#{stage}/db-and-shared/scheduled; quit\" '#{ftp_server}'"
    end
end
...
namespace :JuntoDeploy do
    task :SetLocalConfiguration, :roles => :app do
        ...
        set :db_username, config["#{stage}_db_username"].to_s
        set :db_password, config["#{stage}_db_password"].to_s
        run "lftp -u '#{ftp_username}','#{ftp_password}' -e \"cd /remoteftp/junto/sensitiveconfig; get wp-sensitive-#{stage}.json -o #{release_path}/config/sensitive/wp-sensitive-local.json; quit\" '#{ftp_server}'"
...
namespace(:deploy) do
  ...
  task :backup, :roles => :app do
    ...
    run "lftp -u '#{ftp_username}','#{ftp_password}' -e \"cd /remoteftp/juntobackups/#{stage}/db-and-shared/ondeploy; put #{shared_path}/onDeployDbAndSharedBackup.bf -o onDeployDbAndSharedBackupBefore#{release_name}.bf; quit\" '#{ftp_server}'"
  end
end
